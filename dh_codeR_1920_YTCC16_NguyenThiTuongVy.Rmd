...
dh_data_1920_YTCC_NguyenThiTuongVy <- read_excel("...")
...
Figure 1. Histograms, scatter plots, and Correlation coefficients  between weather conditions and hospitalization in HCM, Vietnam, 2017-2019
```{r}
require("psych")
...
############################################################
## DLNM modeling3###########################################
Fit the DLNM model with a priori parameters
```{r}
...
# Hospitalization Coefs and SD (1oC above MMT==26)
```{r}
...
**Plot the overall cumulative cure between the main model and the alternative models**
```{r}
...
```
**Compare Q-AIC between main model and alternative models**
```{r}
# Q-AIC FUNCTION
...
**Sensitivity modeling checking 2**
  **Change the df of time-trend and seasonality controlling**
```{r}
# MAIN MODEL
# - PREDICTOR SPACE: QUADRATIC SPLINE WITH SPECIFIC KNOT SELECTION
# - LAG SPACE: NATURAL CUBIC SPLINE WITH DF AT EQUALLY-SPACED LOG-VALUES
...
**plot the overall cumulative curve between the main model and the alternative models**
```{r}
par(mar=c(5,4,1,1)+0.1,cex.axis=0.9,mgp=c(2.5,1,0))
#layout(matrix(1:2,ncol=2))
...
## Heatwave analysis
load packages, functions, and createa matrix for storing results
```{r}
# LOAD PACKAGES AND FUNCTIONS
library(dlnm) ; library(splines) ; library(mgcv);require(MASS);require(lmtest)
...
# FUNCTION TO CREATE A HEAT WAVE INDICATOR FOR A TEMPERATURE SERIES
